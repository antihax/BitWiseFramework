...
set profileFolder=%DAYZ%\mpmissions\dayzOffline.chernarusplus\profiles
...
start %DAYZ%\DayZDiag_x64.exe "-server" "-limitFPS=200" -mod=%currentMod%%allclientmods%" "-cpuCount=32"  "-config=%configPath%" "-profiles=%profileFolder%" -doLogs -newErrorsAreWarnings=1 -nosplash -noPause -filePatching -scrDef=BITWISEDEBUG
 
rem start %DAYZSERVER%\DayZServer_x64.exe "-server" "-mod=%currentMod%%allclientmods%" "-cpuCount=32"  "-config=%configPath%" "-profiles=%profileFolder%"  -filePatching 
rem start %DAYZ%\DayZDiag_x64.exe "-mod=%currentMod%%allclientmods%" "-cpuCount=32" "-connect=127.0.0.1" "-port=2302" "-profiles=%profileFolderClient%" -dologs -newErrorsAreWarnings=1  -nosplash -noPause -filePatching -scrDef=BITWISEDEBUG